...
Every ordered list item starts with literal `1.`, not ascending `1. 2. 3.`. This minimizes diff noise when items are added, removed, or reordered: a single insertion in a `1. 2. 3.` list rewrites every subsequent number. The linter accepts both styles (markdownlint MD029 default `one_or_ordered`); the project convention is the all-`1.` form per `AGENTS.md`. Flag ascending-numbered lists with a suggestion block converting them to `1.`.
