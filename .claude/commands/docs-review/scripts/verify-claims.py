...
2. **GitHub via `gh`** (pass1 lane) — `gh_query` for anything `pulumi/*` OR `pulumi-labs/*` ships. Pulumi HCL specifically lives under `pulumi-labs/pulumi-hcl`, and other in-progress providers / SDK experiments ship under `pulumi-labs/*` too; when a claim mentions Pulumi HCL by name (or references a `pulumi-labs/<repo>` package), query BOTH owners before considering escalation:
   ...
   - **Linked implementing change** — when the claim is about a NEW pulumi symbol (a flag, command, or API) you cannot find on the default branch, AND this PR cites an implementing change (a "This docs PR cites implementing change(s)" line in the user message, or a `pulumi/<repo>#<n>` / `github.com/pulumi/<repo>/(pull|commit)/...` reference), READ it before giving up: `gh pr diff <n> -R pulumi/<repo>` or `gh api repos/pulumi/<repo>/commits/<sha>`. Docs that ship alongside the feature are the normal case — the symbol lives in the linked PR, not yet on `master`.
   `gh` results count as `high` confidence when they directly match — they read source-of-truth. Don't loop `issues`/`pulls` for *blind* context discovery — but an implementing PR THIS docs PR explicitly cites is not discovery; read it. A brand-new symbol confirmed only in a linked, not-yet-merged implementing PR is `verified` at confidence `medium`, with `evidence` noting "confirmed in linked impl PR <ref>, not yet on default branch / released" — do NOT return `unverifiable` merely because the symbol isn't in the released CLI or the published docs reference yet. "Not in the published reference" is a docs lag, not a doubt, once you have read the implementing code. Keep your `gh_query` + `read_file` calls under 8 total; if you can't close the claim, return `unverifiable` (or, from a pass1 lane, set `route_escalation: "pass3"` when a public web source plausibly could resolve it).
