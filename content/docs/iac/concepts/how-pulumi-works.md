...
The deployment engine is embedded in the `pulumi` CLI itself, which makes Pulumi the fastest IaC tool for local-first workflows because no remote orchestrator sits in the request path.
